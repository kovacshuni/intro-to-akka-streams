...
class BoundedRejectQueueSpec extends Specification {
  ...
  "size" should {
    ...
    "keep up with many instructions" in {
      ...
      val q = BoundedRejectWorkQueue[Int](n)
      val k1 = 1000
      (1 to k1).foreach(i => q.add(Work(i)))
      val k2 = 500
      (1 to k2).foreach { i =>
        q.add(Work(i))
        q.poll
      ...
      val k3 = 20000
      (1 to k3).foreach(i => q.add(Work(i)))
      q.size must beEqualTo(k1 + k2 + k3)
      val k4 = 20000
      (1 to k4).foreach(i => q.add(Work(i)))
  ...
  "Adding" should {
    "reject elements after bound reached." in {
      ...
      val q = BoundedRejectWorkQueue[Int](n)
      (1 to n) foreach { i =>
        q.add(Work(i)) must beTrue
      }
      (1 to n) foreach { i =>
        q.add(Work(i)) must beFalse
      }
      q.add(Work(n + 1)) must beFalse
    }
  }
  ...
  "Polling" should {
    "return None after emptied." in {
      ...
      (1 to n).foreach(i => q.add(Work(i)))
      (1 to n).foreach { i =>
        q.poll must beEqualTo(Some(Work(i)))
      }
      (1 to n).foreach { i =>
